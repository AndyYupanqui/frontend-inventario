...
export class VentaDetailComponent implements OnInit {
  ...
  declararVenta() {
    Swal.fire({
      ...
      if (result.value) {
        var ruc = "20601473047";
        var tipo_comprobante = this.comprobante.toLowerCase();
        // var productos = Object.assign({}, this.productos);
        var serie= "E001";
        var numero = this.idParam;
        var fecha = this.fecha;
        var importe_total = this.monto1;
        ...
        var fecha_emision = fecha.split("/")[2] + "-" + mes + "-" + dia;
        ...
        this.socket.emit('ENVIAR_DECLARACION', JSON.stringify(comprobante));

        this.__ventaService.actualizarEstadoVenta(this.idParam).subscribe(
          response => {
  ...
  listarVentaProducto() {
    ...
    this.__ventaService.buscarVenta(this.idParam).subscribe(
      response => {
        ...
        this.comprobante = response.orden_venta[0].comprobante;
        this.comprobante = this.comprobante[0].toUpperCase() + this.comprobante.slice(1);
        this.estado_declaracion = response.orden_venta[0].estado_declaracion;
      },
      error => {
        console.log(error);
      }
